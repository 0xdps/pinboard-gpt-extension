...
console.log(`📦 Prepending ${browser} browser code to background.js...`);
...
try {
  ...
  // Prepend the browser code to the background script
  const enhancedBackground = browserContent + '\n\n' + backgroundContent;
  ...
  console.log(`✅ Successfully prepended ${browser} browser code to background.js`);
  console.log(`📄 Final background.js size: ${Math.round(enhancedBackground.length / 1024)}KB`);
  
} catch (error) {
  console.error('❌ Error prepending browser code:', error.message);
  process.exit(1);
}