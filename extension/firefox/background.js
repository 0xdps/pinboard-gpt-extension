...
function removeContextMenu() {
  browser.contextMenus.remove("pin-selection", () => {
    if (browser.runtime.lastError) {
      console.log('Context menu not found or error:', browser.runtime.lastError.message);
    }
  });
}
...
browser.runtime.setUninstallURL('https://gptpins.dps.codes/goodbye.html');
...
browser.contextMenus.onClicked.addListener((info, tab) => {
  ...
  if (info.menuItemId === 'pin-selection') {
    ...
    browser.tabs.sendMessage(tab.id, { action: 'ping' }, (pingResponse) => {
      if (browser.runtime.lastError) {
        ...
        browser.scripting.executeScript({
          ...
          if (browser.runtime.lastError) {
            console.error('GPT Pinboard: Failed to inject content script:', browser.runtime.lastError.message);
            return;
          }
...
browser.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg?.action === 'open-and-highlight' && msg.pin) {
    ...
    browser.tabs.query({ active: true, currentWindow: true }, (activeTabs) => {
      ...
      browser.tabs.query({ url: targetUrl }, (exactTabs) => {
        if (browser.runtime.lastError) {
          sendResponse({ ok: false, error: browser.runtime.lastError.message });
          return;
        }
        ...
        if (exactTabs && exactTabs.length > 0) {
          ...
          browser.tabs.query({ url: ['https://chatgpt.com/*', 'https://chat.openai.com/*'] }, (chatTabs) => {
            if (chatTabs && chatTabs.length > 0) {
              // Use existing ChatGPT tab but navigate to the correct URL
              const t = chatTabs[0];
              browser.windows.update(t.windowId, { focused: true }, () => {
                browser.tabs.update(t.id, { active: true, url: targetUrl }, () => {
              ...
              browser.tabs.create({ url: targetUrl }, (newTab) => {
