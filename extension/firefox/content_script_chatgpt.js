...
function createPinButtonForMessage(messageContainer) {
  try {
    ...
    if (!runtime?.getURL) {
      console.warn('GPT Pinboard: Extension context not available');
      return null;
    }
    ...
    try {
      ...
      console.warn('GPT Pinboard: Error setting up button container:', error);
      return null;
    }
    ...
    console.warn('GPT Pinboard: Error creating pin button:', error);
    return null;
  }
}
...
function addPinButtonToPopup(popupContainer) {
  ...
  try {
    if (browser.runtime?.getURL) {
      ...
      iconElement.src = browser.runtime.getURL('icons/icon-16.png');
